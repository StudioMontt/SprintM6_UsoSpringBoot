function calcularMonto(dctoInst) {
    return function() {
        ...
        const dctoPorcentaje = parseFloat(select.options[select.selectedIndex].getAttribute(`data-${dctoInst.toLowerCase()}`));

        const montoDcto = sueldoImponible * (dctoPorcentaje / 100);
        document.getElementById(`monto${dctoInst}`).value = montoDcto.toFixed(0);

        const ttalDescuento = parseFloat(document.getElementById('montoAfp').value) + parseFloat(document.getElementById('montoSalud').value);
